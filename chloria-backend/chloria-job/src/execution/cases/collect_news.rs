...
pub(crate) type CollectNewsCaseOutput = (usize, usize);
...
impl LocalCase for CollectNewsCase {
    ...
    async fn execute(self) -> Result<Self::Output> {
        ...
        for new_fetcher in self.news_fetchers {
                ...
                .fetch_news(Box::new(move |article| {
                    ...
                    tokio::task::spawn_local(async move {
                        ...
                        let image_path = match article.image_url {
                            Some(image_url) => {
                                if let Ok(image_bytes) = http_helper.get(&image_url).await {
                                    file_storage
        ...
        let inserted_news_count = tokio::spawn(async move {
            // Save news to the database
            let mut inputs = vec![];
            let mut inserted_news_count = 0;
            while let Some(input) = receiver.recv().await {
                inputs.push(input);
                if inputs.len() >= self.insert_batch_size {
                    let inputs = inputs.drain(..).collect();
...
mod tests {
    ...
    async fn check_required_duration() -> Result<()> {
        // For simpler calculations, ensure `CASES_NUM` > `CASE_PERMITS_NUM`, `PAGES_NUM * PAGE_NEWS_NUM` > `TASK_PERMITS_NUM`
        // and `NEWS_LOAD_DURATION` > `PAGE_LOAD_DURATION`
