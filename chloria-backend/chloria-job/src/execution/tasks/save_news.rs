...
impl Task for SaveNewsTask {
    ...
    async fn perform(&self) -> Result<Self::Output> {
        if let Some(image_url) = &self.fetch_news_output.image_url {
            let file = FileEntity::new(self.fetch_news_output.id.clone(), self.fetch_news_output.published_time);
            let image_bytes: Vec<u8> = self.http_helper.get(&image_url).await?;
            self.file_storage
                .upload_file(UploadFileInput {
                    kind: FileObjectKind::Origin,
                    source_code: self.fetch_news_output.source_code.clone(),
                    key: file.key,
                    bytes: image_bytes,
