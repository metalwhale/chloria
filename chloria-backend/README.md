...
- `execution` module (application business rules): contains a factory, use cases ("cases" for short), ports and tasks
...
- The data flows in this order: `interface` -> `execution / domain` -> `infrastructure`:
  ...
  - `interface` is only aware of factory and use cases, while `infrastructure` is only aware of ports
